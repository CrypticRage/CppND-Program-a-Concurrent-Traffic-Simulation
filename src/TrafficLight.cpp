...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::mt19937 eng(rd());
    std::uniform_real_distribution<float> distr(MIN_CYCLE_TIME, MAX_CYCLE_TIME);

    int cycleTime = distr(eng) * 1000;
    int elapsedTime = 0;

    std::cout << "Starting with cycle time: " << cycleTime << std::endl;
    ...
    while(true) {
        ...
        if (elapsedTime >= cycleTime) {
            ...
            std::cout << "Starting with cycle time: " << cycleTime << std::endl;
            std::cout << "Light: " << _currentPhase << std::endl;
        }
